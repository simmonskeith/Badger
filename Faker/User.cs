    ...
    public static class User
    {
        ...
        private static string GeneratePassword(int length, int nonAlphaNumericChars)
        {
            ...
            while (i < length - 1)
            {
                j = 0;
                flag = false;
                temp = Number.RandomNumber(0, length-1);
                for (j = 0; j < length; j++)
                {
                    if (temp == pos[j])
                    {
            ...
            {
                pass[i] = allowedChars[Number.RandomNumber(0, allowedChars.Length-1)];
            }

            // Random the NonAlphaNumericChars
            for (i = length - nonAlphaNumericChars; i < length; i++)
            {
                pass[i] = allowedNonAlphaNum[Number.RandomNumber(0, allowedNonAlphaNum.Length-1)];
            }
